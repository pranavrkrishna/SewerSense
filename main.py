...
@app.post("/api/com_port")
async def update_com_port(update: ComPortUpdate):
    global serial_worker
    if serial_worker:
        serial_worker.stop()
    
    logger.info(f"Switching Serial Worker to {update.port}...")
    serial_worker = SerialWorker(
        port=update.port,
        baudrate=DEFAULT_BAUD_RATE,
        data_callback=handle_new_distance
    ...
    serial_worker.start()
    return {"status": "success", "port": update.port}
